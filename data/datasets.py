...
from .utils.data import random_rotate_z, normalize_pc, augment_pc, combine_pcs
import json
...
@DATASETS.register_module()
class cap3d_ensemble(data.Dataset):
    def __init__(self, config):
        logging.debug(f'Loading csv data from {config.CSV_PATH}.')

        self.pc_path_root = config.PC_PATH_ROOT
        df = pd.read_csv(config.CSV_PATH)
        self.file_list = df['data_path'].tolist()

        self.openshape_setting = config.openshape_setting
        self.npoints = config.npoints
        self.transforms = config.train_transform
        ...
        print('LOADED DATASET!!! samples: ', len(self.file_list))

        self.combine_prob = 0.5
    ...
    def load_pc(self, path):
        pc_data = np.load(path, allow_pickle=True).item()
        xyz = pc_data['xyz']
        rgb = pc_data['rgb']
        pc = torch.from_numpy(np.concatenate((xyz, rgb), axis=1)).float()

        pc[:, [1, 2]] = pc[:, [2, 1]]

        return pc, pc_data['blip_caption' if torch.rand(1) < 0.5 else 'msft_caption']


    def __getitem__(self, idx):
        combine = torch.rand(1) < self.combine_prob
        while True:
            try:
                captions = []
                point_path = self.file_list[idx]
                if combine:
                    idxs = [idx]
                    #choose if to use 2 or 3 pc
                    num_pc = 2 #if torch.rand(1) < 0.5 else 3
                    pcs = []
                    #generate the random attributes for the combined pc
                    attributes = []
                    for i in range(num_pc):
                        # size can be small, normal, or large
                        size = "" #if torch.rand(1) < 0.33 else "large" if torch.rand(1) < 0.66 else "small"
                        if i == 0:
                            position = (None, None)
                            pc, caption = self.load_pc(point_path)
                            if caption == '' or caption == ' ':
                                raise ValueError("empty caption")
                            captions.append(caption)
                            pcs.append(pc.transpose(0,1))
                        else:
                            # choose next to or over
                            position = "next to" if torch.rand(1) < 0.5 else "over"
                            # choose to which pc to be next to or over, possible positions are numbers between 0 and num_pc and excluding the current i
                            other_object_index = torch.arange(0, num_pc).tolist()
                            other_object_index.remove(i)
                            if i == 2 and attributes[i-1]["position"][1] in other_object_index:
                                other_object_index.remove(attributes[i-1]["position"][1])
                            idx_other = torch.randint(0, len(other_object_index), (1,)).item()
                            position = (position, other_object_index[idx_other])
                            # load the random pc
                            new_obj_idx = torch.randint(0, len(self.file_list), (1,)).item()
                            pc, caption = self.load_pc(self.file_list[new_obj_idx])
                            if caption == '' or caption == ' ':
                                raise ValueError("empty caption")
                            captions.append(caption)
                            idxs.append(new_obj_idx)
                            pcs.append(pc.transpose(0,1))

                        attributes.append({"size": size, "position": position})
                    if num_pc == 2:
                        order = [1, 0]
                    else:
                        order = [1, 2, 0] if position[1] == 0 else [2, 1, 0]
                    
                    data = combine_pcs(pcs, self.sample_points_num, 300, "random", attributes)

                    data = data.transpose(0,1)

                else:
                    data, caption = self.load_pc(point_path)  # shape (N, 6)
                    captions.append(caption)
                
                if data.shape[0] != self.npoints:
                    data = self.random_sample(data.transpose(0,1),self.npoints) # shape (npoints, 6)
                rgb = data[:, 3:].numpy().astype(np.float32)
                data = data[:,:3].numpy().astype(np.float32)
            except Exception as e:
                print(f"Catched exception: {str(e)}. Re-trying…")
                #print(os.path.join(self.pc_path_root, self.file_list[new_obj_idx] + '.pt'))
                print(point_path)
                # import time
                # time.sleep(1)
            else:
        ...
        if combine:
            #handle the combine of the captions: two kinds of combinations: 1. "a *size* *caption1* *position1* a *size0* *caption0* (and so on if there are 3 objects)"
            #2 "a *size0* *caption0* and a *size1* *caption1* (and so on if there are 3 objects). The first object is *position0* the secoond (and so on if there are 3 objects)"
            # remember to remove "a" or "an" from the captions if they start with it

            #randomly choose the type 1 or 2 combination
            comb_type = 0 if torch.rand(1) < 0.5 else 1
            comb_caption = ""
            if comb_type:
                #first find the correct order by starting from the PC which is not pointed by any other PC as next to or over
                # iterate with the order defined above
                for o in order:
                    #remove "a" and "an" if the caption starts with them, independently of the lowercase or not
                    caption = captions[o]
                    if caption[:2].lower() == "a ":
                        caption = caption[2:]
                    elif caption[:3].lower() == "an ":
                        caption = caption[3:]
                    if caption[-1] == '.':
                        caption = caption[:-1]
                    size = attributes[o]['size'] + " " if attributes[o]['size'] != "" else ""
                    comb_caption += f"a {size}{caption}"
                    if o != order[-1]:
                        comb_caption += f" {attributes[o]['position'][0]} "
                    else:
                        comb_caption += "."
            else:
                #iterate with the order defined above
                ...
                for i, o in enumerate(order):
                    ...
                    if o != order[-1]:
                        ...
                        comb_caption += "."
                    if i == 0:
                        last_sentence += " The first object is " + attributes[o]['position'][0] + " "
                    elif i == 1:
                        if len(order) == 3:
                            last_sentence += "the second object. The second is " + attributes[o]['position'][0] + " "
                        else:
                            last_sentence += "the second."
                    else:
                        last_sentence += "the third."
                comb_caption += last_sentence
        ...
        if self.return_caption_raw:
            return data, caption_emb, rgb, caption
        return data, caption_emb, rgb
...
class cap3d(data.Dataset):
    def __init__(self, config):
        ...
        self.pc_path_root = config.PC_PATH_ROOT
        df = pd.read_csv(config.CSV_PATH)
        self.file_list = df['filenames'].tolist()
        ...
        print('LOADED DATASET!!! samples: ', len(self.captions))
    ...
    def __getitem__(self, idx):
        ...
        while True:
            try:
                point_path = os.path.join(self.pc_path_root, self.file_list[idx] + '.pt')
                if combine:
                    idxs = [idx]
                    #choose if to use 2 or 3 pc
                    num_pc = 2 if torch.rand(1) < 0.5 else 3
                    ...
                    for i in range(num_pc):
                        ...
                        if i == 0:
                            ...
                            # load the random pc
                            new_obj_idx = torch.randint(0, len(self.file_list), (1,)).item()
                            pc = torch.load(os.path.join(self.pc_path_root, self.file_list[new_obj_idx] + '.pt'))
                            idxs.append(new_obj_idx)
                            pcs.append(pc)
                    ...
                    data = torch.load(point_path)  # shape (6, N)
                data = self.random_sample(data.transpose(0,1),self.npoints) # shape (npoints, 6)
                rgb = data[:, 3:].numpy().astype(np.float32) / 255.0
                data = data[:,:3].numpy().astype(np.float32)
            except Exception as e:
                print(f"Catched exception: {str(e)}. Re-trying…")
                #print(os.path.join(self.pc_path_root, self.file_list[new_obj_idx] + '.pt'))
...
# r = data[3, :] / 255.0  # Normalize RGB values to [0, 1]
# g = data[4, :] / 255.0
# b = data[5, :] / 255.0
#
