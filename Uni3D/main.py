...
def main(args):
    ...
    train_dataset = get_dataset(train_transform, tokenizer_clip, args, 'train')
    val_dataset = get_dataset(None, tokenizer_clip, args, 'val')
    val_dataset_lvis = get_dataset(None, tokenizer_clip, args, 'val_lvis')
    val_dataset_scanobjnn = get_dataset(None, tokenizer_clip, args, 'val_scanobjnn')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # if epoch == 1:
        #     logging.info('Setting combine to 0.5')
        #     train_loader.dataset.combine_prob = 0.5

        train_stats = train(train_loader, clip_model, model, criterion, optimizer, scaler, scheduler, epoch, args, device)
        val_stats = {"acc1": -1}

        scaler_state = None if scaler is None else scaler.state_dict()

        with amp.autocast(enabled=not args.disable_amp):
            val_stats = test_zeroshot_3d_core(val_loader, args.validate_dataset_name, model, clip_model, tokenizer_clip, args, "modelnet")
            logging.info(val_stats)
            val_lvis_stats = test_zeroshot_3d_core(val_lvis_loader, args.validate_dataset_name_lvis, model, clip_model, tokenizer_clip, args, "lvis")
            logging.info(val_lvis_stats)
            val_scanobjnn_stats = test_zeroshot_3d_core(val_scanobjnn_loader, args.validate_dataset_name_scanobjnn, model, clip_model, tokenizer_clip, args, 'scanobjnn')
...
def train(train_loader, clip_model, model, criterion, optimizer, scaler, scheduler, epoch, args, device):
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        if not args.use_embed:
            texts = compute_embedding(clip_model, texts, None, device)

        inputs = [feature, texts]
        # to device
